# ahp_final_ui_refactored_with_goal.py
...
# Page config
# ----------------------------
st.set_page_config(page_title="AHP Pro - Refactored (with Goal)", layout="wide")
...
def priority_from_matrix(M):
    ...
    col_sum = np.where(col_sum == 0, 1e-12, col_sum)
    norm = M / col_sum
    pr = norm.mean(axis=1)
    return pr, norm
...
def to_excel_bytes(dfs: dict):
    """
    Export dict of DataFrames to Excel bytes.
    Tanpa engine, agar kompatibel di Streamlit Cloud.
    """
    out = BytesIO()
    with pd.ExcelWriter(out) as writer:  # tanpa engine
...
if 'goal_name' not in st.session_state:
    st.session_state.goal_name = "Tujuan AHP"
...
max_k = st.sidebar.number_input(
    "Jumlah Kriteria (max)",
    min_value=2, max_value=20, value=8, step=1,
    disabled=disabled_sidebar, key="sb_max_k"
)
max_sub = st.sidebar.number_input(
    "Max Subkriteria per Kriteria",
    min_value=0, max_value=15, value=5, step=1,
    disabled=disabled_sidebar, key="sb_max_sub"
)
max_alt = st.sidebar.number_input(
    "Jumlah Alternatif (max)",
    min_value=1, max_value=50, value=5, step=1,
    disabled=disabled_sidebar, key="sb_max_alt"
)
...
# header reset
st.markdown("<h1 style='text-align:center;'>AHP Pro — Refactored (with Goal)</h1>", unsafe_allow_html=True)
...
# ----------------------------
# Tab: Struktur (with Goal)
# ----------------------------
with tabs[0]:
    st.header("1. Struktur Hierarki AHP")

    if not st.session_state.structure_done:
        with st.form("form_structure", clear_on_submit=False):
            # Goal (Tujuan)
            st.subheader("Tujuan (Goal)")
            goal = st.text_input(
                "Nama Tujuan AHP",
                value=st.session_state.goal_name,
                key="form_goal_name"
            )
            goal = goal.strip() if goal.strip() else "Tujuan AHP"
            ...
            for i in range(int(n_k)):
                ...
                cnt = st.number_input(
                    f"Subkriteria untuk {k_names[i]}",
                    min_value=0, max_value=max_sub,
                    value=default_sc, step=1,
                    key=sc_key
                )
            ...
            n_alt = st.number_input(
                "Jumlah Alternatif",
                min_value=1, max_value=max_alt,
                value=default_n_alt, step=1,
                key="form_n_alt"
            )
        ...
        if submit_struct:
            # Save structure and clear derived results
            st.session_state.structure_done = True
            st.session_state.goal_name = goal
        ...
        st.write("**Tujuan**:", st.session_state.goal_name)
        st.write("**Kriteria**:", st.session_state.k_names)
        st.write("**Sub counts (per kriteria)**:", st.session_state.sub_counts)
...
with tabs[1]:
    ...
    if not st.session_state.structure_done:
        ...
        st.info(f"**Tujuan:** {st.session_state.goal_name}")
...
with tabs[4]:
    ...
    if not st.session_state.structure_done:
        ...
        st.subheader("Tujuan AHP")
        st.write(st.session_state.goal_name)
        ...
        if 'alt_prior' in st.session_state and global_sub:
            ...
            if final_scores.sum() > 0:
                ...
                to_export = {"Goal": pd.DataFrame({"Goal":[st.session_state.goal_name]}), "GlobalWeights": df_gl, "FinalRanking": dff}
                ...
                st.download_button(
                    "Unduh Hasil (.xlsx)",
                    data=to_excel_bytes(to_export),
                    file_name='AHP_results_with_goal.xlsx',
